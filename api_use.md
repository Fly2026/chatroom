登录
说明：登录有五个接口使用 对密码编码或者使用 请求，避免某些特殊字符无法解析,如#(#在 url 中会被识别为 hash,而不是 query)encodeURIComponentPOST
...
code: 验证码，使用 /captcha/sent接口传入手机号获取验证码,调用此接口传入验证码,可使用验证码登录

可选参数

userid: 用户 id,当用户存在多个账户是 时，必须加上需要登录的用户 id
...
刷新登录
说明 : 调用此接口，可刷新登录状态，可以延长 过期时间token
...
获取用户云盘音乐 URL
说明：登录后调用此接口可以获取用户上传到云盘的音乐 URL，部分可以直接用 直接获取 URL（需要登录，目前获取到的文件大小都约为 10M 左右）/song/url
...
收藏歌单/新建歌单
说明 : 调用此接口, 可收藏歌单/新建歌单( 需要登录 ), 收藏成功后建议使用 /playlist/tracks/add 把原歌单下的歌曲添加到新的歌单
...
调用例子： /playlist/tracks/add?listid=1&data=我们应该算爱过吧|8E10D8825DDE03BCABBDE13E5A4150D2 /playlist/tracks/add?listid=1&data=我们应该算爱过吧|8E10D8825DDE03BCABBDE13E5A4150D2|67026620|477417208 /playlist/tracks/add?listid=1&data=我们应该算爱过吧|8E10D8825DDE03BCABBDE13E5A4150D2,我们应该算爱过吧|5015FC3FAB5B0C245556A1CC3C4DE355
...
新碟上架
说明: 调用此接口 , 可获取新碟上架列表, 如需要专辑详细信息需要调用album/detail, 如需要获取专辑音乐列表需调 用album/songs
...
fields: 需要返回的信息，可以传多个，以逗号分割，支持的值有 trans_param special_tag authors album_name publish_date cover intro publish_company type album_id language_id is_publish heat grade quality exclusive grade_count author_name sizable_cover language category
...
获取音乐 URL（新版）
说明: 调用此接口, 传入的音乐 hash, 可以获取对应的音乐的 url, 未登录状态或者非会员可能会返回为空，该接口会一次性返回支持的音质的音频 url, 但该接口存 在音频加密（目前无法解码），请谨慎使用
...
man: 是否返回多个歌词，：返回多个， ：返回一个。 默认为yesnono
...
category_id: tag，0：推荐，11292：HI-RES，其他可以从 /playlist/tags 接口中获取（接口下的 为 的值）tag_idcategory_id
...
主题歌单
说明 : 调用此接口 , 可获取主题歌单, 通过 /theme/playlist/track 可以获取主题个单下的歌曲
...
ids: 歌单中的 ，可以传多个，用逗号分隔global_collection_id
...
调用例子： /playlist/similar?ids=collection_1_1341266283_964007_0 /playlist/similar?ids=collection_1_1341266283_964007_0,collection_3_1041185112_11_0
...
card_id: 1：对应安卓 精选好歌随心听 || 私人专属好歌，2：对应安卓 经典怀旧金曲，3：对应安卓 热门好歌精选，4：对应安卓 小众宝藏佳作，5：未知，6：对应 vip 专属推荐
...
调用例子： /images?hash=B04ED0F01ABBB62B9D22EC4616ED8AFE /image?hash=B04ED0F01ABBB62B9D22EC4616ED8AFE,55603312694BF99AD6000C2D0D72D368&album_id=,75013431
...
调用例子： /images/audio?hash=B04ED0F01ABBB62B9D22EC4616ED8AFE /image/audio?hash=B04ED0F01ABBB62B9D22EC4616ED8AFE,55603312694BF99AD6000C2D0D72D368
...
songId：音乐 songid, 该字段需要请求 获取音乐伴奏信息 获取

singerName：歌手名称，多个以 隔开，也可以到 获取音乐伴奏信息 中获取、

songHash：音乐 hash, 该字段需要请求 获取音乐伴奏信息 获取
...
调用例子： /privilege/lite?hash=B04ED0F01ABBB62B9D22EC4616ED8AFE /privilege/lite?hash=B04ED0F01ABBB62B9D22EC4616ED8AFE,55603312694BF99AD6000C2D0D72D368
...
fields: 可以传 , , , , 每个 field 以逗号分开album_infoauthors.basebaseaudio_infoauthors.ipextratagstagmap
...
编辑精选专区
说明：调用此接口，可获取编辑精选专区相关内容，若数据中没有 ip_id，可使用/ip/zone/home来获取数据
...
编辑精选专区详情
说明：调用此接口，可获取编辑精选专区详情，若/ip/zone 数据中没有 ip_id，可使用该接口获取数据
...
rank_cid：若需要返回往期歌曲列表，则该参数为必填，否则默认返回最新一期，/rank/vol 返回值中， 则为该参数volid
...
频道 - 详情
说明：调用此接口，传入 可以获取频道详情global_collection_id / channel_id
...
频道 - 频道安利
说明：调用此接口，传入 可以获取频道安利global_collection_id / channel_id
...
频道 - 相似频道
说明：调用此接口，传入 可以获取相似频道global_collection_id / channel_id
...
频道 - 订阅
说明：登录后调用此接口， 传入 可订阅频道global_collection_id / channel_id
...
频道 - 音乐故事
说明：调用此接口，传入 可以获取音乐故事global_collection_id / channel_id
...
频道 - 音乐故事详情
说明：调用此接口，传入 和 可以获取音乐故事详情global_collection_id / channel_idfileid
